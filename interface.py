...
class MainWindow(QMainWindow, Ui_MainWindow):
        ...
        try:
            ...
            if 'xls' in filePath or 'xlsx' in filePath:
                ...
                if self.design_location_df.shape[0]>0:
                    format_data(writer=writer, df=self.design_location_df, sheet_name='A1.1-库位推荐', source_data_info=data_info)
                if self.current_location_df.shape[0] > 0:
                    format_data(writer=writer, df=self.current_location_df, sheet_name='A1.2-现状库位', source_data_info=data_info)
                if self.inventory_customer_df.shape[0] > 0:
                    format_data(writer=writer, df=self.inventory_customer_df, sheet_name='A2-客户货型分布', source_data_info=data_info)
                if self.inventory_age_df.shape[0] > 0:
                    format_data(writer=writer, df=self.inventory_age_df, sheet_name='A3-库龄', source_data_info=data_info)
                if self.inv_df.shape[0] > 0:
                    format_data(writer=writer, df=self.inv_df, sheet_name='01-数据源')
                if self.sku_df.shape[0] > 0:
                    format_data(writer=writer, df=self.sku_df, sheet_name='02-sku维度数据源')
    ...
    def outbound_data_process(self):
        try:
            ...
            self.textBrowser.setText('1. in outbound_data_process function ~~~~~~~~~~')
            ...
            # row1 = self.ob_df.shape[0]
            self.textBrowser.setText('2. 有效列名截取成功 ~~~~~~~~~')
            self.textBrowser.setText(str(detail_data.columns))
            ...
            self.textBrowser.setText('2-1 填充空白时间')
            for col in date_col:
                print('修改时间列时间格式： ', col)
                try:
                    ...
                    pass
            self.textBrowser.setText('2-2 修改填充时间格式')
            ## 新增 客户代码列
            ...
            self.textBrowser.setText('3. 订单明细数据处理完成 ~~~~~~~~~~')
            ...
            self.textBrowser.setText('4. 订单流入时间数据处理完成 ~~~~~~~~~~')
        ...
        try:
            ...
            if 'xls' in filePath or 'xlsx' in filePath:
                ...
                if self.normal_order_EIQ.shape[0]>0:
                    format_data(writer=writer, df=self.normal_order_EIQ, sheet_name='B1.1-标准订单EIQ', source_data_info=outbound_source_info)
                if self.fba_order_EIQ.shape[0] > 0:
                    format_data(writer=writer, df=self.fba_order_EIQ, sheet_name='B1.2-FBA EIQ', source_data_info=outbound_source_info)
                if self.order_type_EIQ.shape[0] > 0:
                    format_data(writer=writer, df=self.order_type_EIQ, sheet_name='B1.3-订单结构EIQ', source_data_info=outbound_source_info)
                if self.wave_order_type_EIQ.shape[0] > 0:
                    format_data(writer=writer, df=self.wave_order_type_EIQ, sheet_name='B1.4-波次EIQ', source_data_info=outbound_source_info)
                if self.current_pick_order_EIQ.shape[0] > 0:
                    format_data(writer=writer, df=self.current_pick_order_EIQ, sheet_name='B1.5-拣货EIQ', source_data_info=outbound_source_info)
                if self.multi_order_EIQ.shape[0] > 0:
                    format_data(writer=writer, df=self.multi_order_EIQ, sheet_name='B2-多品订单组合', source_data_info=outbound_source_info)
                if self.customer_sales.shape[0] > 0:
                    format_data(writer=writer, df=self.customer_sales, sheet_name='B3-客户动销', source_data_info=outbound_source_info)
                if self.sku_active_sales.shape[0] > 0:
                    format_data(writer=writer, df=self.sku_active_sales, sheet_name='B4-SKU动销', source_data_info=outbound_source_info)
                if self.sku_hour_in_df.shape[0] > 0:
                    format_data(writer=writer, df=self.sku_hour_in_df, sheet_name='B5-SKU小时流入', source_data_info=outbound_source_info)
                if self.sku_ABC_df.shape[0] > 0:
                    format_data(writer=writer, df=self.sku_ABC_df, sheet_name='B6.1-ABC分类', source_data_info=outbound_source_info + self.abc_info)
                if self.sku_ABC_detail.shape[0] > 0:
                    format_data(writer=writer, df=self.sku_ABC_detail, sheet_name='B6.2-SKU ABC分类明细', source_data_info=outbound_source_info + self.abc_info)
                if self.sku_age_df.shape[0] > 0:
                    format_data(writer=writer, df=self.sku_age_df, sheet_name='B7.1-SKU出库库龄', source_data_info=outbound_source_info)
                if self.order_age_df.shape[0] > 0:
                    format_data(writer=writer, df=self.order_age_df, sheet_name='B7.2-订单出库库龄', source_data_info=outbound_source_info)
                if self.channel_order_distribution.shape[0] > 0:
                    format_data(writer=writer, df=self.channel_order_distribution, sheet_name='B8.1-渠道分布', source_data_info=outbound_source_info)
                if self.platform_order_distribution.shape[0] > 0:
                    format_data(writer=writer, df=self.platform_order_distribution, sheet_name='B8.2-平台分布', source_data_info=outbound_source_info)
        ...
        try:
            ...
            if 'xls' in filePath or 'xlsx' in filePath:
                ...
                if self.ib_date_distribution.shape[0]>0:
                    format_data(writer=writer, df=self.ib_date_distribution, sheet_name='C1-入库量分布', source_data_info=self.ib_info)
                if self.ib_container_carton_type.shape[0] > 0:
                    format_data(writer=writer, df=self.ib_container_carton_type, sheet_name='C2-箱型分布', source_data_info=self.ib_info)
                if self.ib_container_skuNum.shape[0] > 0:
                    format_data(writer=writer, df=self.ib_container_skuNum, sheet_name='C3-单箱SKU分布', source_data_info=self.ib_info)
                if self.ib_daily_container_num.shape[0] > 0:
                    format_data(writer=writer, df=self.ib_daily_container_num, sheet_name='C4-日来柜数量', source_data_info=self.ib_info)
